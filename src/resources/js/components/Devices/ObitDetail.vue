<template>
    <div>
        ...
        <ul v-if="localObit != null" class="device-information-list py-5">
            <device-row :bold_title="true"  :title="'Owner'" :value="localObit.owner_did"></device-row>
        </ul>

        <h2>Device Identification</h2>
        <ul v-if="localObit != null" class="device-information-list py-5">
            <device-row :bold_title="true" :title="'Universal Serial Number'" :value="localObit.usn"></device-row>
            <device-row :bold_title="true" :classes="{'lower':true}"  :title="'ObitDID'" :value="localObit.did"></device-row>
        ...
        <table v-if="localObit != null" class="table table-bordered mt-5">
            <tbody>
            <tr  v-show="blockchainObit !== null && blockchainObit.checksum.length > 0">
                <td>
            ...
            <tr>
                <td>
                    <button v-show="blockchainObit === null || localObit.checksum != blockchainObit.checksum" class="btn btn-primary btn-round" @click="syncData">UPLOAD TO BLOCKCHAIN</button>
                    <button v-show="blockchainObit !== null && blockchainObit.checksum != ''" class="btn btn-primary btn-round" @click="downloadObit">DOWNLOAD FROM BLOCKCHAIN</button>
                </td>
            </tr>
